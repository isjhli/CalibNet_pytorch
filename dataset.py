...
class KITTIFilter:
    ...
    Args:
        ...
        concat (str, optional): concat operation for normal estimation,
                                "none", "xyz" or "zero-mean". Defaults to "none".
    """
    ...
    def __call__(self, x: np.ndarray):
        ...
        if self.concat == "none":
            ...
            elif self.concat == "zero-mean":
                center = np.mean(pcd_xyz, axis=0, keepdims=True)
...
class BaseKITTIDataset(Dataset):
    ...
    @staticmethod
    def check(odom_obj: pykitti.odometry, cam_id: int, seq: str):
        ...
        assert cam_files_length == velo_files_length, head_msg + ("number of cam %d (%d) and velo files (%d) doesn't "
                                                                  "equal!") % (
                                                          cam_id, cam_files_length, velo_files_length)
    ...
    def __getitem__(self, index):
        ...
        T_cam2velo = getattr(data.calib, "T_cam%d_velo" % self.cam_id)  # 雷达坐标系到相继坐标系，p_l -> p_c
        ...
        _calibed_pcd = self.pcd_tran(calibed_pcd.T).T  # (3, N)
        *_, rev = transform.binary_projection((REVH, REVW), K_cam_extend, _calibed_pcd)
        _calibed_pcd = _calibed_pcd[:, rev]  # 落在扩展图像视野范围内的点云
        _calibed_pcd = self.resample_tran(_calibed_pcd.T).T  # [3, n]
        _pcd_range = np.linalg.norm(_calibed_pcd, axis=0)  # [n, ]，点云的模长，即点云的距离
        ...
        return dict(img=_img, pcd=_calibed_pcd, pcd_range=_pcd_range, depth_img=_depth_img, InTran=K_cam,
                    ExTran=T_cam2velo)
